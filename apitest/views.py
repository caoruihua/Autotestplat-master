...
def apitest_manage(request):
    ...
    return render ( request, "apitest_manage.html", {"user": username, "apitests": apitest_list,
                                                     "apitestcounts": apitest_count} )  # 把值赋给apitestcounts这个变量
...
def test_report(request):
    ...
    db = pymysql.connect ( user='root', db='autotest', passwd='058t25402', host='cdb-rl6yi7n0.bj.tencentcdb.com',
                           port=10128 )
    cursor = db.cursor ()
...
def tasksearch(request):
    ...
    crontab_list = CrontabSchedule.objects.all ()  # 定时任务 （如：某年月日的某时，每天的某时）
    return render ( request, 'periodic_task.html',
                    {"user": username, "tasks": task_list, "periodics": periodic_list, "crontabs": crontab_list} )
